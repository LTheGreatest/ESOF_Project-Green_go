...
void main() {
  ...
  group('LocationService', ()
  {
    ...
    test(
        'checkIfLocationServiceIsEnabled() returns true if location service is enabled', () async {
      when(mockGeolocator.isLocationServiceEnabled()).thenAnswer((
          _) async => true);
    ...
    test(
        'checkLocationPermission() returns correct value based on permission status', () async {
      when(mockGeolocator.checkPermission()).thenAnswer((
    ...
    test(
        'requestLocationPermission() requests permission and returns correct value', () async {
      when(mockGeolocator.checkPermission()).thenAnswer((
          _) async => LocationPermission.denied);
